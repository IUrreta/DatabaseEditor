...
class DatabaseUtils:
    ...
    def fetch_engines(self):
        ...
        for i in range(len(engines_ids)):
            ...
            for stat in stats_ids:
                res = self.cursor.execute("SELECT UnitValue FROM Parts_Designs_StatValues WHERE DesignID = " + str(engines_ids[i]) + " AND PartStat = " + str(stat)).fetchone()
                statList.append(res[0])
            ers_res = self.cursor.execute("SELECT UnitValue FROM Parts_Designs_StatValues WHERE DesignID = " + str(ers_ids[i]) + " AND PartStat = 15").fetchone()
            statList.append(ers_res[0])
            gearbox_res = self.cursor.execute("SELECT UnitValue FROM Parts_Designs_StatValues WHERE DesignID = " + str(gearboxes_ids[i]) + " AND PartStat = 15").fetchone()
    ...
    def fetch_driverContract(self, id):
        details = self.cursor.execute("SELECT Salary, EndSeason, StartingBonus, RaceBonus, RaceBonusTargetPos FROM Staff_Contracts WHERE ContractType = 0 AND StaffID = " + str(id)).fetchone()
        return details
    ...
    def fetch_seasonResults(self, yearSelected): 
        year =  (yearSelected, )
        drivers = self.cursor.execute("SELECT DriverID FROM Races_DriverStandings WHERE RaceFormula = 1 AND SeasonID = " + str(year[0])).fetchall()
        seasonResults = []
    ...
    def fetch_oneTeam_seasonResults(self, team, year):
        drivers = self.cursor.execute("SELECT DISTINCT DriverID FROM Races_Results WHERE Season = " + str(year[0]) + " AND TeamID = " + str(team[0])).fetchall()
    ...
    def fetch_oneDriver_seasonResults(self, driver, year):
        results = self.cursor.execute("SELECT DriverID, TeamID, FinishingPos, Points FROM Races_Results WHERE Season = " + str(year[0]) + " AND DriverID = " + str(driver[0])).fetchall()
        if results:
            sprintResults = self.cursor.execute("SELECT RaceID, FinishingPos, ChampionshipPoints FROM Races_SprintResults WHERE SeasonID = " + str(year[0]) + " AND DriverID = " + str(driver[0])).fetchall()
            teamID = results[0][1]
            driverName = self.cursor.execute("SELECT FirstName, LastName FROM Staff_BasicData WHERE StaffID = " + str(driver[0])).fetchone()
    ...
    def fetch_predictable_events_from(self, year):
        ...
        season_events = self.cursor.execute("SELECT TrackID FROM Races WHERE SeasonID = " + str(year) + " AND RaceID <= " + str(last_predictable[0])).fetchall()
        tuple_numbers = {num for tpl in season_events for num in tpl}
        season_ids = self.cursor.execute("SELECT RaceID FROM Races WHERE SeasonID = " + str(year) + " AND RaceID <= " + str(last_predictable[0])).fetchall()
        events_ids =[]
    ...
    def fetch_events_done_from(self, year):
        day_season = self.cursor.execute("SELECT Day, CurrentSeason FROM Player_State").fetchone()
        season_ids = self.cursor.execute("SELECT RaceID FROM Races WHERE SeasonID = " + str(year) + " AND Day < " + str(day_season[0])).fetchall()
        events_ids =[]
        for i in range(len(season_ids)):
    ...
    def fetch_events_from(self, year):
        season_events = self.cursor.execute("SELECT TrackID FROM Races WHERE SeasonID = " + str(year)).fetchall()
        tuple_numbers = {num for tpl in season_events for num in tpl}

        season_ids = self.cursor.execute("SELECT RaceID FROM Races WHERE SeasonID = " + str(year)).fetchall()
        events_ids =[]
        for i in range(len(season_ids)):
            events_ids.append((season_ids[i][0], season_events[i][0]))
    ...
    def format_seasonResults(self, results, driverName, teamID, driverID, year, sprints):
        ...
        races_participated = self.cursor.execute("SELECT RaceID FROM Races_Results WHERE DriverID = " + str(driverID[0]) + " AND Season = " + str(year[0])).fetchall()
        formatred_results = [(result[-2], result[-1]) for result in results]
        for i in range(len(races_participated)):
            driver_with_fastest_lap = self.cursor.execute("SELECT DriverID FROM Races_Results WHERE FastestLap > 0 AND RaceID = "+ str(races_participated[i][0]) + " AND Season = " + str(year[0]) + " ORDER BY FastestLap LIMIT 1; ").fetchone()
            driver_with_pole = self.cursor.execute("SELECT DriverID FROM Races_QualifyingResults WHERE QualifyingStage = 3 AND FinishingPos = 1 AND RaceID = " + str(races_participated[i][0])).fetchone()
            dnfd = self.cursor.execute("SELECT DNF FROM Races_Results WHERE DriverID = " + str(driverID[0]) + " AND Season = " + str(year[0]) + " AND RaceID = " + str(races_participated[i][0])).fetchone()
            ...
            if driver_with_fastest_lap[0] == driverID[0]:
                ...
                formatred_results[i] = tuple(results_list)
            QStage = self.cursor.execute("SELECT MAX(QualifyingStage) FROM Races_QualifyingResults WHERE RaceFormula = 1 AND RaceID =" + str(races_participated[i][0]) + " AND SeasonID = " + str(year[0]) + " AND DriverID = " + str(driverID[0])).fetchone()
            QRes = self.cursor.execute("SELECT FinishingPos FROM Races_QualifyingResults WHERE RaceFormula = 1 AND RaceID =" + str(races_participated[i][0]) + " AND SeasonID = " + str(year[0]) + " AND DriverID = " + str(driverID[0]) + " AND QualifyingStage = " + str(QStage[0])).fetchone()
            results_list = list(formatred_results[i])
        ...
        for i in range(len(formatred_results)):
            team_in_race = self.cursor.execute("SELECT TeamID FROM Races_Results WHERE RaceID = " + str(formatred_results[i][0]) + " AND DriverID = " + str(driverID[0])).fetchone()
            formatred_results[i] += (team_in_race)

        
        position = self.cursor.execute("SELECT Position FROM Races_Driverstandings WHERE RaceFormula = 1 AND SeasonID = " + str(year[0]) + " AND DriverID = " + str(driverID[0])).fetchone()
    ...
    def fetch_drivers_per_year(self, year):
        drivers = self.cursor.execute('SELECT  bas.FirstName, bas.LastName, res.DriverID, res.TeamID FROM Staff_BasicData bas JOIN Races_Results res ON bas.StaffID = res.DriverID WHERE Season = ' + str(year) + " GROUP BY bas.FirstName, bas.LastName, bas.StaffID, res.TeamID ORDER BY res.TeamID").fetchall()
        formatted_tuples = []
    ...
    def fetch_calendar(self):
        day_season = self.cursor.execute("SELECT Day, CurrentSeason FROM Player_State").fetchone()
        calendar = self.cursor.execute("SELECT TrackID, WeatherStateQualifying, WeatherStateRace, WeekendType, State FROM Races WHERE SeasonID = " + str(day_season[1])).fetchall()
        return calendar

    def check_claendar(self):
        default_tracks = [2, 1, 11, 24, 22, 5, 6, 4, 7, 10, 9, 12, 13, 14, 15, 17, 19, 18, 20, 21, 23, 25, 26]
        day_season = self.cursor.execute("SELECT Day, CurrentSeason FROM Player_State").fetchone()
        season_events = self.cursor.execute("SELECT TrackID FROM Races WHERE SeasonID = " + str(day_season[1])).fetchall()
        tuple_numbers = {num for tpl in season_events for num in tpl}
        first_race = self.cursor.execute("SELECT MIN(Day) FROM Races WHERE SeasonID = " + str(day_season[1])).fetchone()
        last_race_last_season = self.cursor.execute("SELECT MAX(RaceID) FROM Races WHERE SeasonID = " + str(day_season[1]-1)).fetchone()
        first_race_curr_season = self.cursor.execute("SELECT MIN(RaceID) FROM Races WHERE SeasonID = " + str(day_season[1])).fetchone()


        season_ids = self.cursor.execute("SELECT RaceID FROM Races WHERE SeasonID = " + str(day_season[1])).fetchall()
        events_ids =[]
        for i in range(len(season_ids)):
            events_ids.append((season_ids[i][0], season_events[i][0]))
    ...
    def format_names_get_stats(self, name, type):
        ...
        if type == "driver":
            stats = self.cursor.execute("SELECT Val FROM Staff_PerformanceStats WHERE StaffID = " + str(name[2]) + " AND StatID BETWEEN 2 AND 10").fetchall()
            additionalStats = self.cursor.execute("SELECT Improvability, Aggression FROM Staff_DriverData WHERE StaffID = " + str(name[2])).fetchone()
            nums = resultado + tuple(stat[0] for stat in stats) + additionalStats

            return nums

        elif type == "staff1":
            stats = self.cursor.execute("SELECT Val FROM Staff_PerformanceStats WHERE StaffID = " + str(name[2]) + " AND StatID IN (0,1,14,15,16,17);").fetchall()
            nums = resultado + tuple(stat[0] for stat in stats)

            return nums

        elif type == "staff2":
            stats = self.cursor.execute("SELECT Val FROM Staff_PerformanceStats WHERE StaffID = " + str(name[2]) + " AND StatID IN (13,25,43);").fetchall()
            nums = resultado + tuple(stat[0] for stat in stats)

            return nums

        elif type == "staff3":
            stats = self.cursor.execute("SELECT Val FROM Staff_PerformanceStats WHERE StaffID = " + str(name[2]) + " AND StatID IN (19,20,26,27,28,29,30,31);").fetchall()
            nums = resultado + tuple(stat[0] for stat in stats)

            return nums

        elif type == "staff4":
            stats = self.cursor.execute("SELECT Val FROM Staff_PerformanceStats WHERE StaffID = " + str(name[2]) + " AND StatID IN (11,22,23,24);").fetchall()
            nums = resultado + tuple(stat[0] for stat in stats)
