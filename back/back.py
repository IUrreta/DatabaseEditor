...
async def handle_command(message):
    ...
    if type == "connect":
        ...
        contractDetails = dbutils.fetch_driverContract(message["driverID"])
        contractMsg = [contractDetails]
        contractMsg.append(dbutils.fetchDriverNumberDetails(message["driverID"]))
        ...
        results = dbutils.fetch_seasonResults(message["year"])
        results.insert(0, dbutils.fetch_events_from(message["year"]))
        results.insert(0, "Results fetched")
        results.append(dbutils.fetch_teamsStadings(message["year"]))
        ...
        drivers = dbutils.fetch_drivers_per_year(message["year"])
        ...
        for id in message["graph"]:
            if(message["mode"] == "driver"):
                res = dbutils.fetch_oneDriver_seasonResults((id,), (message["year"],))
                h2hDrivers.append(res)
            elif(message["mode"] == "team"):
                res = dbutils.fetch_oneTeam_seasonResults((id,), (message["year"],))
                h2hDrivers.append(res)
        h2hDrivers.append(dbutils.fetch_events_done_from(message["year"]))
        h2hDrivers.insert(0, dbutils.fetch_events_from(message["year"]))
        ...
        events = [dbutils.fetch_predictable_events_from(message["year"])]
        ...
        pred_msg = ["Prediction Fetched", dbutils.fetch_next_race(), prediction]
